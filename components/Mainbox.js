...
function Mainbox(props) {
  ...
  useEffect(() => {
   ...
   noResults?ParaDisplay.style.display="block":ParaDisplay.style.display="none";
                ...
                props.stockdata.map((item, index) => (
                  <div className={styles.showdata} id="show-data" key={index}>
                    ...
                    <span className={styles.change}>
                      ...
                      <span id="pricechange" style={item.change>=0? {color:"green"}:{color:"red"}}>{item.change.toFixed(2)}</span>
                    </span>
                    <span className={styles.pChange} id="pc">
                      <div className={styles.infotext}>PROFIT/LOSS</div>
                      <span style={item.pChange>=0? {color:"green"}:{color:"red"}}>{item.pChange.toFixed(2)}%</span>
                    </span>
                  </div>
                ))
              )}
